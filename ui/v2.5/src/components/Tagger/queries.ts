...
export const useUpdatePerformerStashID = () => {
  ...
  const updatePerformerHandler = (performerID: string, stashID: string, endpoint: string) => (
    updatePerformer({
      variables: {
        id: performerID,
        stash_ids: [{
      ...
      update: (store, updatedPerformer) => {
        ...
        store.writeQuery<GQL.FindPerformersQuery, GQL.FindPerformersQueryVariables>({
  ...
  const handleCreate = (performer: GQL.PerformerCreateInput, stashID: string) => (
    createPerformer({
      ...
      update: (store, newPerformer) => {
        ...
        const currentQuery = store.readQuery<GQL.AllPerformersForFilterQuery, GQL.AllPerformersForFilterQueryVariables>(
          {
        ...
        if (allPerformersSlim.length > 1) {
          store.writeQuery<GQL.AllPerformersForFilterQuery, GQL.AllPerformersForFilterQueryVariables>({
            query: GQL.AllPerformersForFilterDocument,
            data: {
              allPerformersSlim,
            },
  ...
  const handleCreate = (studio: GQL.StudioCreateInput, stashID: string) => (
    createStudio({
      ...
      update: (store, result) => {
        ...
        const currentQuery = store.readQuery<GQL.AllStudiosForFilterQuery, GQL.AllStudiosForFilterQueryVariables>(
          {
        ...
        const allStudiosSlim = sortBy(
          [
            ...
            result.data.studioCreate,
          ],
          ["name"]
        );
        if (allStudiosSlim.length > 1) {
          store.writeQuery<GQL.AllStudiosForFilterQuery, GQL.AllStudiosForFilterQueryVariables>({
            query: GQL.AllStudiosForFilterDocument,
            data: {
              allStudiosSlim,
            },
  ...
  const handleCreate = (tag: string) => (
    createTag({
      ...
      update: (store, result) => {
        ...
        const currentQuery = store.readQuery<GQL.AllTagsForFilterQuery, GQL.AllTagsForFilterQueryVariables>({
        ...
        store.writeQuery<GQL.AllTagsForFilterQuery, GQL.AllTagsForFilterQueryVariables>({
          query: GQL.AllTagsForFilterDocument,
          data: {
            allTagsSlim,
          },
